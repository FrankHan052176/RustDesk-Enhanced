...
//!
//! The HarmonyOS facade describes every session in one vocabulary: start it,
//! report a snapshot, send pointer and keyboard events, and exchange clipboard
//! text. That vocabulary follows the RustDesk protocol because it came first.
//! VNC speaks a different protocol but exposes the same operations through this
//! module. Operations with different semantics are answered explicitly (see
//! [`SessionBackend::set_image_quality`] and [`SessionBackend::peer_info`])
...
mod tests {
    ...
    fn a_vnc_backend_reports_itself_in_the_shared_vocabulary() {
        let (backend, _server) = vnc_backend_over(crate::vnc::scripted::ScriptedFrame {
            width: 2,
            height: 2,
