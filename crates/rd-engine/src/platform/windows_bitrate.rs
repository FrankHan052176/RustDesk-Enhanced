...
mod tests {
    ...
    fn a_long_lull_lowers_the_rate_a_step_at_a_time() {
        ...
        assert!(
            changes >= 2,
            "expected at least one step per window, got {changes}"
        );
    ...
    fn the_rate_does_not_step_faster_than_the_encoder_can_settle() {
        ...
        assert!(
            changes > 1,
            "the scenario must actually produce repeated changes"
        );
    }
    ...
    fn raising_does_not_discard_the_evidence_that_the_budget_is_too_large() {
        ...
        assert!(
            lowered,
            "a long quiet stretch after a raise must lower the rate"
        );
    }
